...
- The REST trust fence follows browser semantics only: requests from non-HTTP transports (third-party desktop shells / custom-protocol carriers dispatching app:// fetches) carry no Host header and are accepted only without an Origin and not labeled cross-site (local non-browser caller semantics, issue #11); carriers that do not forward request bodies (no stream, no body) fail with field-validation errors instead of crashing
- Deleting a bundle plugin no longer swallows a failed/timed-out live unmount: the removal output says "live unmount did not complete — restart to fully apply" (a half-disposed loader tree can leave the web server down while the process lingers, and the automatic-restart chain may then hang; pull it up manually — issue #10; the root fix belongs to the host flush timeout fallback)
